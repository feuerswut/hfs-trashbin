# hfs-trashbin

A plugin for [HFS](https://github.com/rejetto/hfs) that intercepts file deletions and moves them to a trash folder instead of permanently removing them.

## General notice
- This plugin is in beta. I cannot guarantee it to work without tests by other users. Please report back to me if you encounter any issues.
- If the HFS API changes in the future, the plugin may break. Reference version: 3.1.7-3.2.0 (Node 20-24)
- Naturally, keep this off of critical infrastructure.

- GENERAL WARNING: DELETING THIS PLUGIN WILL ERASE ALL DELETED FILES PERMANENTLY. This is, right now, intended.
- There is a sub-folder ./storage/trash in the plugins folder containing ALL DELETED FILES AND FOLDERS.

## Features

- Intercepts HFS delete operations — files are moved to trash, not destroyed
- Per-user trash: each user only sees and manages their own deleted files
- Trash button in the menu bar with a list dialog (restore or permanently delete)
- SQLite-backed record keeping (uses Node.js built-in `node:sqlite`; falls back to sql.js automatically in 3.1.* and prior HFS)
- No configuration required

## Installation

Go to the install plugins panel and simply search for `trashbin`, by feuerswut (me) and click install.

HFS will pick it up automatically. No restart needed.

## Requirements

- Node.js v22.5.0 (HFS 3.2) or later for the built-in SQLite backend (recommended)
- Node.js < 22.5: the plugin will automatically run `install.js` to set up the sql.js fallback. (on older HFS versions)
Make sure that `sqljs.tar.gz` (from the [sql.js releases page](https://github.com/sql-js/sql.js/releases)) is next to `install.js` before starting the plugin. (Should come bundled with the plugin)

## How it works

The plugin hooks into the HFS `deleting` backend event. When a user deletes a file:

1. The file is moved to `<storageDir>/trash/<timestamp>_<filename>`
2. A record is written to `trash.db` with the user, original path, trash path, and deletion timestamp (all paths and the usernames are base64-encoded)
3. HFS receives a `200 OK` response — the frontend sees a successful deletion

The trash folder and database live in the plugin's storage directory, which is preserved across plugin updates.

## Trash UI

A **Trash** button appears in the menu bar for every logged-in user. Clicking it opens a dialog showing all files the current user has trashed, with:

- **Restore** — moves the file back to its original location (fails gracefully if something already exists there)
- **Delete** — permanently removes the file from the trash

## Troubleshooting

If `install.js` fails, ensure `sqljs.tar.gz` is present next to it and re-run manually:

```sh
node install.js
```

Report any errors to me!

## TODO (Features)
[   ] Advanced Trashbin page UI (filters, sorting)

[   ] HFS frontend: popup entry limit

[   ] (?) Admin uninstall warning
